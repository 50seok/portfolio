...
- **개발 기간** : 2026.06.17 ~ 2026.06.24 (머신러닝 개인 프로젝트)
- **담당 역할** : 전 과정 단독 수행
...
## 3. FishCheck — 어종 판별 사기 방지 서비스

> 물고기 사진 한 장으로 광어·가자미를 판별해 어시장 사기를 막는 AI 서비스

"좌광우도"(눈 위치로 광어/가자미 구분) 원리를 딥러닝으로 학습해, 사진을 올리면 어종을 자동 판별합니다. CLIP으로 물고기 사진 여부를 먼저 검증한 뒤 YOLOv8로 어종을 탐지합니다.

- **개발 기간** : 2026.06.24 ~ 2026.07.01 (딥러닝 개인 프로젝트)
- **담당 역할** : 전 과정 단독 수행 (모델 학습 → 배포)
  - Roboflow 데이터셋 구축 (442장 + 3배 증강)
  - YOLOv8s 객체 탐지 모델 학습 (640×640, 4클래스)
  - CLIP 기반 비(非)물고기 이미지 사전 검증
  - Streamlit 앱 + Streamlit Cloud 배포
- **기술 스택** : `Python` `PyTorch` `YOLOv8` `CLIP` `Streamlit`
- **링크** : [GitHub](https://github.com/50seok/FishCheck)
